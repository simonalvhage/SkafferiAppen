...
export default function SkafferiScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const json = await apiGetProducts(apiKey);
  ...
  const handleAddManual = async () => {
    ...
    try {
      await apiAddProduct(name, name, apiKey);
      setSearchText('');
      Keyboard.dismiss();
      await fetchData();
    } catch (error) {
      ...
      [
        {
          ...
          onPress: async () => {
            try {
              await apiAddProduct(item.EAN, item.product, apiKey, 'shopping');
              await apiDeleteProduct(item.EAN, apiKey);
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.emptyContainer}>
          <Icon name="home" size={48} color={COLORS.textLight} />
          <Text style={styles.emptyTitle}>Mitt Skafferi</Text>
          <Text style={styles.emptyText}>
            Logga in eller skapa konto för att se ditt skafferi
          </Text>
          <TouchableOpacity
          ...
          ListEmptyComponent={
            <View style={styles.emptyListContent}>
              ...
              <Text style={styles.emptyListText}>
                {searchText ? 'Inga matchande produkter' : 'Ditt skafferi är tomt'}
              </Text>
            </View>
          }
